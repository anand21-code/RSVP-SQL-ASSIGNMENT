...
select 
  count(*)- count(id) as NULL_ID_SUM, 
  count(*)- count(title) AS NULL_TITLE_SUM, 
  count(*)- count(year) AS NULL_YEAR_SUM, 
  count(*)- count(date_published) AS NULL_DATE_PUBLISHED_SUM, 
  count(*)- count(duration) AS NULL_DURATION_SUM, 
  count(*)- count(country) AS NULL_COUNTRY_SUM, 
  count(*)- count(worlwide_gross_income) AS NULL_WORLDWIDE_GROSS_INCOME_SUM, 
  count(*)- count(languages) AS NULL_LANGUAGES_SUM, 
  count(*)- count(production_company) AS NULL_PRODUCTION_COMPANY_SUM 
from 
  movie;
...
SELECT 
  Count_of_genre, 
  COUNT(movie_id) as Count_of_movie 
FROM 
  (
    SELECT 
      movie_id, 
